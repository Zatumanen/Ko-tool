import{createAudioContext,processAudio,getPreset}from './audio/processor.js?v=20260921-14';
...
function renderFileResult(item){const list=$('results-list');if(list.querySelector('.empty-results'))list.innerHTML='';const row=document.createElement('div');row.className='result-item';const name=document.createElement('span');name.className='result-name';const ext='.wav';name.textContent=item.file.name.replace(/\.[^.]+$/,'')+'_x2'+ext;const preview=document.createElement('button');preview.type='button';preview.className='download';preview.textContent='Preview';preview.onclick=async()=>{try{const openPreview=await loadPreview();openPreview(item,{state,saveBlob,esc,createAudioContext});}catch(e){showError(e?.message||e);}};const dl=document.createElement('button');dl.className='download';dl.type='button';dl.textContent='Download';dl.onclick=()=>saveBlob(item.result.blob,name.textContent);row.append(name,preview,dl);list.appendChild(row);}
