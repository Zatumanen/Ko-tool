import{connectEp133}from './ep133/index.js';
import{initEp133Browser}from './ep133/ui.js?v=20260922-2';
...
window.addEventListener('DOMContentLoaded',()=>{
  document.getElementById('my-ep-icon')?.addEventListener('click',()=>{
    const w=window.open(new URL('./',document.baseURI).href,'my-ep');
    if(w)w.focus();
  });
  document.getElementById('error-ok')?.addEventListener('click',hideError);
  document.getElementById('error-close')?.addEventListener('click',hideError);
  initEp133Browser({showError});
  connectEp133().catch(error=>showError(error?.message||error));
});