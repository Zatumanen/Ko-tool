import{IDENTITY_SYSEX,TE_SYSEX_GREET,TE_SYSEX_FILE,TE_SYSEX_FILE_LIST,TE_SYSEX_FILE_GET,STATUS_OK}from './constants.js';
...
export function requestRead(command,payload=new Uint8Array(),timeout=5000){
  if(command!==TE_SYSEX_FILE_LIST&&command!==TE_SYSEX_FILE_GET&&command!==TE_SYSEX_FILE){
    return Promise.reject(new Error(`EP-133 read-only command rejected: ${command}`));
  }
