...
export function initEp133Browser({showError}={}){
  ...
  const memory=createSampleMemory({
    ...
    onMove:async(source,target)=>{if(!isConnected())throw new Error('Connect EP-133 before moving a sample.');if(!soundsParentId)throw new Error('EP-133 /sounds destination is not available. Refresh the device.');try{setSlotStatus(source,'MOVING TO '+String(target.id).padStart(3,'0')+'...');await moveFile(source.nodeId,soundsParentId,target.id);await readDevice();setStatus('MOVED · '+String(source.id).padStart(3,'0')+' → '+String(target.id).padStart(3,'0'));}catch(error){showError?.(error?.message||error);setSlotStatus(source,'MOVE ERROR');}},
